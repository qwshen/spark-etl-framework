...
object ArgumentParser extends Loggable {
  ...
  private case class Configuration(pipelineDef: String, applicationConf: String,
    variables: Map[String, String] = Map.empty[String, String], stagingUri: Option[String] = None, stagingActions: Seq[String] = Nil, metricsLoggingUri: Option[String] = None, metricsLoggingActions: Seq[String] = Nil)
  ...
  def parse(args: Array[String]): Arguments = scoptParser.parse(args, Configuration("", "")) match {
    case Some(cfg) => {
      ...
      val stagingBehavior: Option[StagingBehavior] = (cfg.stagingUri, cfg.stagingActions) match {
        case (Some(_), Seq(_, _ @ _*)) => Some(StagingBehavior(true, cfg.stagingUri, cfg.stagingActions))
        case _ => None
      }
      ...
      val metricsLoggingBehavior: Option[MetricsLogging] = (cfg.metricsLoggingUri, cfg.metricsLoggingActions) match {
        case (Some(_), Seq(_, _ @ _*)) => Some(MetricsLogging(true, cfg.metricsLoggingUri, cfg.metricsLoggingActions))
        case _ => None
      }
      //arguments
      Arguments(config, cfg.pipelineDef, stagingBehavior, metricsLoggingBehavior)
    }
