...
class Encryptor {
  ...
  /*
    Run encryption
   */
  def execute(args: Array[String]): (String, String) = scoptParser.parse(args, Configuration(None, None, "")) match {
...
object Encryptor {
  def main(args: Array[String]): Unit = {
    Try((new Encryptor()).execute(args)) match {
      case Success(x) => System.out.println(new SecurityChannel(x._1).encrypt(x._2))
      case _ =>
        System.out.println("Usage: java -cp ./spark-etl-framework-1.0.jar com.qwshen.Encryptor --key-string key-value --data data-value")
        System.out.println("Usage: java -cp ./spark-etl-framework-1.0.jar com.qwshen.Encryptor --key-file file-name --data data-value")
    }
  }
}